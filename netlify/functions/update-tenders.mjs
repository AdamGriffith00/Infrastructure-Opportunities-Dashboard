...
export async function handler() {
  try {
    ...
    // Fetch CF + FTS (broad, minimal filtering to maximise volume)
...
// ===== Contracts Finder (OCDS Search) with pagination =====
...
// ===== Find a Tender (OCDS) with cursor pagination =====
async function fetchFTS() {
  ...
  for (let i = 0; i < maxBatches; i++) {
    ...
    // Find the next cursor from Link header (if present)
...
function parseNextCursor(bodyText = "") {
  // crude but works: look for &cursor=... in the response body (FTS puts it in their links)
  const m = bodyText.match(/cursor=([A-Za-z0-9+/=%-]+)/);
  return m ? decodeURIComponent(m[1]) : "";
}
function parseNextCursorFromData(d = {}) {
  // sometimes FTS includes a "next" link
  const link = d.links?.next || d.link?.next;
  if (!link) return "";
  const m = String(link).match(/cursor=([A-Za-z0-9+/=%-]+)/);
  return m ? decodeURIComponent(m[1]) : "";
}
